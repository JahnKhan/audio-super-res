...
def add_data(h5_file, inputfiles, args, save_examples=False):
  ...
  with open(inputfiles) as f:
    for line in f:
      ...
      if ext in file_extensions:
        file_list.append(os.path.join(args.in_dir, filename).replace("\\","/"))
  ...
  for j, file_path in enumerate(file_list):
    ...
    #C:\Users\Zocker\Documents\Projekte\Uni\saywerb\audio-super-res\data\grocery
    # load audio file
  ...
  if args.dimension is not -1:
    ...
    # pickle the data
    cPickle.dump(hr_patches, open('full-label-'+args.out[:-7],'wb'))
    cPickle.dump(lr_patches, open('full-data-'+args.out[:-7],'wb'))
    cPickle.dump(ID_list, open('ID_list','wb'))
